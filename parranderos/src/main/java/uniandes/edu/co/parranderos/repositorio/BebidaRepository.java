...
public interface BebidaRepository extends JpaRepository<Bebida, Integer> {
    ...
    @Query(value = "SELECT * FROM Parranderos.bebidas", nativeQuery = true)
    Collection<Bebida> darBebidas();

    @Query(value = "SELECT * FROM Parranderos.bebidas WHERE id = :id", nativeQuery = true)
    Bebida darBebida(@Param("id") long id);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM Parranderos.bebidas WHERE id = :id", nativeQuery = true)
    void eliminarBebida(@Param("id") long id);

    @Modifying
    @Transactional
    @Query(value = "UPDATE Parranderos.bebidas SET nombre = :nombre, grado_alcohol = :grado_alcohol, tipo = :tipo WHERE id = :id", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO Parranderos.bebidas (id, nombre, grado_alcohol, tipo) VALUES ( parranderos_sequence.nextval , :nombre, :grado_alcohol, :tipo)", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT B.*\r\n" + //
            ...
            "INNER JOIN Parranderos.sirven SB ON B.ID = SB.ID_BEBIDA\r\n" + //
            "INNER JOIN Parranderos.bares BR ON SB.ID_BAR = BR.ID\r\n" + //
    ...
    @Query(value = "SELECT COUNT(*) AS TOTAL_BEBIDAS, \r\n" + //
            ...
            "FROM Parranderos.bebidas", nativeQuery = true)
